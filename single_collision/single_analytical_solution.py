...
def get_analytical_solution(cfg):
    ...
    q10 = np.array([cfg.init_pos_0[0], cfg.init_pos_0[1]])
    q20 = np.array([cfg.init_pos_1[0], cfg.init_pos_1[1]])
    ...
    angle = np.linspace(0.01, np.pi - 0.01, 100)
    S, ANGLE = np.meshgrid(s, angle)
    S = S.reshape(-1)
    ANGLE = ANGLE.reshape(-1)
    inp = np.stack([S, ANGLE], axis=1)
