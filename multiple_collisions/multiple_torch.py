    ...
    for i in range(2):
        ...
        if penetration_dist_norm < 2 * cfg.radius:
            ...
            if penetration_projected_v < 0.:
                ...
                temp_x_inc[s_id] = torch.min(toi - dt, torch.tensor(0)) * temp_imp[s_id]          
                x_inc = x_inc + temp_x_inc
    # ball-wall collision
    for i in range(2):
        dist_norm = cfg.wall_y - (x[i, 1] + v[i, 1] * dt)
        rela_v = v[i]
        if dist_norm < cfg.radius:
            dir = torch.tensor([0.0, -1.0])
            projected_v = torch.dot(rela_v, dir)
            if projected_v < 0:
                temp_imp = torch.zeros_like(imp)
                temp_imp[i] = - (1 + cfg.elasticity) * projected_v * dir
                imp = imp + temp_imp
                toi = (dist_norm - cfg.radius) / torch.min(
                    torch.tensor(-1e-3), projected_v
                )
                temp_x_inc = torch.zeros_like(x_inc)
                temp_x_inc[i] = torch.min(toi - dt, torch.tensor(0)) * temp_imp[i]
                x_inc = x_inc + temp_x_inc
