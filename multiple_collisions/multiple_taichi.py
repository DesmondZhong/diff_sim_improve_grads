...
def collide(t: ti.i32):
    ...
    for i in range(2):
        ...
        if penetration_dist_norm < 2 * radius:
            ...
            penetration_projected_v = penetration_dir.dot(rela_v)
    ...
    for i in range(2):
        imp = ti.Vector([0.0, 0.0])
        x_inc_contrib = ti.Vector([0.0, 0.0])
        v_next = v[t, i] + ctrls[t, i] * dt
        dist_norm = ti.abs(x[t, i][1] + v_next[1] * dt - cfg.wall_y)
        rela_v = v[t, i]
        if dist_norm < radius:
...
def forward(cfg, iter=0):
    ...
    for t in range(1, steps + 1):
        ...
        if (t + 1) % interval == 0 and iter % 100 == 0:
            gui.clear()
            gui.line(begin=[-1, (cfg.wall_y + 2.5) / 4], end=[1, (cfg.wall_y + 2.5) / 4])
            gui.circle(fit_to_canvas(goal), 0x0, pixel_radius // 16)
            colors = [0xCCCCCC, 0x3344cc]
            for i in range(2):
                gui.circle(fit_to_canvas(x[t, i]), colors[i], pixel_radius// 4)
...
if __name__ == '__main__':
    # render
    try: 
        gui = ti.GUI("Multiple collisions", (1024, 1024), background_color=0x3C733F)
